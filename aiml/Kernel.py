...
class Kernel:
    ...
    def addPattern(self, pattern, getResponseFunction):
        self._userPatterns[pattern] = getResponseFunction

    def getUserPattern(self, inp, sessionID=_globalSessionID):
        inp = inp.strip()

        for pattern in self._userPatterns.keys():
            p = '^{}$'.format(re.sub('{([^}]*)}', '(?P<\g<1>>.*)', pattern).strip())

            match = re.search(p, inp, flags=re.IGNORECASE)
            kwargs = {}
            if match:
                return lambda: self._userPatterns[pattern](**match.groupdict())
    ...
    def respond(self, input, sessionID = _globalSessionID):
        ...
        finalResponse = ""
        for s in sentences:
            up = self.getUserPattern(s, sessionID)
            if up:
                finalResponse += up() + "  "
                continue
